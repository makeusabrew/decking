...
Cluster.prototype.sortCluster = function(containers) {
  ...
  var depTree = new DepGraph();

  _.each(
    _.pluck(containers, "originalName"),
    depTree.addNode.bind(depTree)
  );

  // @TODO optimise all these loops...
  _.each(containers, function(container) {
    var deps = _.union(container.object.dependencies, container.object['mount-from']);
    _.each(deps, function(dep) {
      depTree.addDependency(container.originalName, dep);
    });
  });

  var self = this;
  var sortedCluster = depTree.overallOrder();

  /*
  _.each(sortedCluster, function(node) {
    console.log("node", node, depTree.dependenciesOf(node));
  });
  */
  var results = _.map(sortedCluster, function(item) {
    return self.findContainer(containers, item);
  });
...
Cluster.prototype.hasDependency = function(containers, dependency) {
  // @TODO: strict check here causes issues, investigate
