...
Decking.prototype.loadConfig = function(file) {
  return Parser.load(JSON.parse((fs.readFileSync(file))));
};
...
Decking.prototype._build = function(image, done) {
  ...
  this.log("Looking up build data for " + image);
  ...
  this.log("Building image " + image + " from " + targetPath);
  ...
  writeStream.on("close", function() {
    ...
    if(self.hasArg("--no-cache")) {
      self.log("Not using image cache");
      options.nocache = true;
    }

    self.log("Uploading compressed context...");
    ...
    return self.docker.buildImage(tar.stdout, options, function(err, res) {
      fs.unlink("./Dockerfile", function(err) {
        if(err) return self.log("[WARN] Could not remove Dockerfile");
      });
...
Decking.prototype._run = function(cmd, done) {
  ...
  if(!cluster) {
    ...
    this.log("Defaulting to cluster '" + cluster + "'");
  }
  ...
  if(target.group) {
    this.log("Using overrides from group '" + target.group + "'\n");
  }
...
/**
 * Creates containers for a cluster.
 *
...
/**
 * Starts containers for a cluster.
 *
 * @param {Object} cluster
 * @param {Function} done
 */
Decking.prototype.start = function(cluster, done) {
  var self = this;
  return self.resolveContainers(this.config, cluster, function(list) {
...
/**
 * Stops containers for a cluster.
 *
 * @param {Object} cluster
 * @param {Function} done
 */
...
/**
 * Restarts containers for a cluster.
 *
 * @param {Object} cluster
 * @param {Function} done
 */
...
/**
 * Gets the status of containers for a cluster.
 *
 * @param {Object} cluster
 * @param {Function} done
 */
...
/**
 * Attaches to containers for a cluster.
 *
 * @param {Object} cluster
 * @param {Function} done
 */
...
/**
 * Determines if the given argument is specified.
 *
 * @param {String} arg
 *
 * @return {Boolean}
...
Decking.prototype.logStream = function(name, data) {
  return this.log("" + (Table.padName(name, "(", ")")) + " " + data);
};
