  ...
  return this.isRunning(container, function(err, running) {
    ...
    return container.start(function(err) {
      ...
      if (details.object.ready) {
        ...
        return container.inspect(function(err, data) {
          awaitPort(details.object.ready, data.NetworkSettings.IPAddress, 200, 10000)
          .then(function() {
            setTimeout(function() {
              self.table.renderOk(name);
              return callback();
      ...
      // @TODO: if we have a `start` option check what it is;
      // if it's a port, check that port
      // if it's a timeout, delay until it's ready
      self.table.renderOk(name);
      return callback();
    });
  });
...
Decking.prototype._destroyIterator = function(details, callback) {
  ...
  var params = {
    force: true,
    // destroy volumes too (@TODO: take this from input)
    v: true
  };
